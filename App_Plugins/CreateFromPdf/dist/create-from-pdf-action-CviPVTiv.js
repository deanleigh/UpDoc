...
var m = (n, e, t) => Fe(n, typeof e != "symbol" ? e + "" : e, t), ye = (n, e, t) => e.has(n) || we("Cannot " + t);
var F = (n, e, t) => (ye(n, e, "read from private field"), t ? t.call(n) : e.get(n)), N = (n, e, t) => e.has(n) ? we("Cannot add the same private member more than once") : e instanceof WeakSet ? e.add(n) : e.set(n, t);
...
import { UmbEntityActionBase as Qe } from "@umbraco-cms/backoffice/entity-action";
...
]`).replace("lheading", Pe).replace("|table", "").replace("blockquote", " {0,3}>").replace("|fences", "").replace("|list", "").replace("|html", "").replace("|tag", "").getRegex() }, gt = /^\\([!"#$%&'()*+,\-./:;<=>?@\[\]\\^_`{|}~])/, kt = /^(`+)([^`]|[^`][\s\S]*?[^`])\1(?!`)/, Be = /^( {2,}|\\)\n(?!\s*$)/, dt = /^(`+|[^`])(?:(?= {2,}\n)|[\s\S]*?(?:(?=[\\<!\[`*_]|\b_|$)|[^ ](?= {2,}\n)))/, te = /[\p{P}\p{S}]/u, de = /[\s\p{P}\p{S}]/u, Ee = /[^\s\p{P}\p{S}]/u, ft = f(/^((?![*_])punctSpace)/, "u").replace(/punctSpace/g, de).getRegex(), Le = /(?!~)[\p{P}\p{S}]/u, xt = /(?!~)[\s\p{P}\p{S}]/u, bt = /(?:[^\s\p{P}\p{S}]|~)/u, mt = f(/link|precode-code|html/, "g").replace("link", /\[(?:[^\[\]`]|(?<a>`+)[^`]+\k<a>(?!`))*?\]\((?:\\[\s\S]|[^\\\(\)]|\((?:\\[\s\S]|[^\\\(\)])*\))*\)/).replace("precode-", et ? "(?<!`)()" : "(^^|[^`])").replace("code", /(?<b>`+)[^`]+\k<b>(?!`)/).replace("html", /<(?! )[^<>]*?>/).getRegex(), De = /^(?:\*+(?:((?!\*)punct)|[^\s*]))|^_+(?:((?!_)punct)|([^\s_]))/, wt = f(De, "u").replace(/punct/g, te).getRegex(), yt = f(De, "u").replace(/punct/g, Le).getRegex(), Ne = "^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)", Tt = f(Ne, "gu").replace(/notPunctSpace/g, Ee).replace(/punctSpace/g, de).replace(/punct/g, te).getRegex(), $t = f(Ne, "gu").replace(/notPunctSpace/g, bt).replace(/punctSpace/g, xt).replace(/punct/g, Le).getRegex(), Rt = f("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)", "gu").replace(/notPunctSpace/g, Ee).replace(/punctSpace/g, de).replace(/punct/g, te).getRegex(), St = f(/\\(punct)/, "gu").replace(/punct/g, te).getRegex(), At = f(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace("scheme", /[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace("email", /[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/).getRegex(), zt = f(ge).replace("(?:-->|$)", "-->").getRegex(), vt = f("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>").replace("comment", zt).replace("attribute", /\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex(), X = /(?:\[(?:\\[\s\S]|[^\[\]\\])*\]|\\[\s\S]|`+[^`]*?`+(?!`)|[^\[\]\\`])*?/, Ct = f(/^!?\[(label)\]\(\s*(href)(?:(?:[ \t]*(?:\n[ \t]*)?)(title))?\s*\)/).replace("label", X).replace("href", /<(?:\\.|[^\n<>\\])+>|[^ \t\n\x00-\x1f]*/).replace("title", /"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/).getRegex(), Me = f(/^!?\[(label)\]\[(ref)\]/).replace("label", X).replace("ref", ue).getRegex(), Oe = f(/^!?\[(ref)\](?:\[\])?/).replace("ref", ue).getRegex(), _t = f("reflink|nolink(?!\\()", "g").replace("reflink", Me).replace("nolink", Oe).getRegex(), Re = /[hH][tT][tT][pP][sS]?|[fF][tT][pP]/, fe = { _backpedal: Z, anyPunctuation: St, autolink: At, blockSkip: mt, br: Be, code: kt, del: Z, emStrongLDelim: wt, emStrongRDelimAst: Tt, emStrongRDelimUnd: Rt, escape: gt, link: Ct, nolink: Oe, punctuation: ft, reflink: Me, reflinkSearch: _t, tag: vt, text: dt, url: Z }, It = { ...fe, link: f(/^!?\[(label)\]\((.*?)\)/).replace("label", X).getRegex(), reflink: f(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label", X).getRegex() }, ie = { ...fe, emStrongRDelimAst: $t, emStrongLDelim: yt, url: f(/^((?:protocol):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/).replace("protocol", Re).replace("email", /[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/).getRegex(), _backpedal: /(?:[^?!.,:;*_'"~()&]+|\([^)]*\)|&(?![a-zA-Z0-9]+;$)|[?!.,:;*_'"~)]+(?!$))+/, del: /^(~~?)(?=[^\s~])((?:\\[\s\S]|[^\\])*?(?:\\[\s\S]|[^\s~\\]))\1(?=[^~]|$)/, text: f(/^([`~]+|[^`~])(?:(?= {2,}\n)|(?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)|[\s\S]*?(?:(?=[\\<!\[`*~_]|\b_|protocol:\/\/|www\.|$)|[^ ](?= {2,}\n)|[^a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-](?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)))/).replace("protocol", Re).getRegex() }, Pt = { ...ie, br: f(Be).replace("{2,}", "*").getRegex(), text: f(ie.text).replace("\\b_", "\\b_| {2,}\\n").replace(/\{2,\}/g, "*").getRegex() }, Q = { normal: ke, gfm: ht, pedantic: ut }, M = { normal: fe, gfm: ie, breaks: Pt, pedantic: It }, qt = { "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }, Se = (n) => qt[n];
function z(n, e) {
  if (e) {
...
`, n = n.substring(d.length + 1), h = !0), !h) {
          let T = this.rules.other.nextBulletRegex(w), I = this.rules.other.hrRegex(w), L = this.rules.other.fencesBeginRegex(w), v = this.rules.other.headingBeginRegex(w), be = this.rules.other.htmlBeginRegex(w);
          for (; n; ) {
            let C = n.split(`
`, 1)[0], A;
            if (d = C, this.options.pedantic ? (d = d.replace(this.rules.other.listReplaceNesting, "  "), A = d) : A = d.replace(this.rules.other.tabCharGlobal, "    "), L.test(d) || v.test(d) || be.test(d) || T.test(d) || I.test(d)) break;
            if (A.search(this.rules.other.nonSpaceChar) >= w || !d.trim()) u += `
` + A.slice(w);
            else {
              if (g || c.replace(this.rules.other.tabCharGlobal, "    ").search(this.rules.other.nonSpaceChar) >= 4 || L.test(c) || v.test(c) || I.test(c)) break;
              u += `
` + d;
            }
            !g && !d.trim() && (g = !0), k += C + `
`, n = n.substring(C.length + 1), c = A.slice(w);
          }
        }
  ...
  table(n) {
    ...
    let t = ze(e[1]), s = e[2].replace(this.rules.other.tableAlignChars, "").split("|"), r = (o = e[3]) != null && o.trim() ? e[3].replace(this.rules.other.tableRowBlankLine, "").split(`
`) : [], i = { type: "table", raw: e[0], header: [], align: [], rows: [] };
    if (t.length === s.length) {
      ...
      for (let l of r) i.rows.push(ze(l, i.header.length).map((p, a) => ({ text: p, tokens: this.lexer.inline(p), header: !1, align: i.align[a] })));
      return i;
    }
  }
  lheading(n) {
...
` ? e[1].slice(0, -1) : e[1];
  ...
  constructor(e) {
    ...
    let t = { other: y, block: Q.normal, inline: M.normal };
    this.options.pedantic ? (t.block = Q.pedantic, t.inline = M.pedantic) : this.options.gfm && (t.block = Q.gfm, this.options.breaks ? t.inline = M.breaks : t.inline = M.gfm), this.tokenizer.rules = t;
  }
  static get rules() {
    return { block: Q, inline: M };
  }
  static lex(e, t) {
...
`;
    return s ? '<pre><code class="language-' + z(s) + '">' + (t ? r : z(r, !0)) + `</code></pre>
...
}, m(H, "passThroughHooks", /* @__PURE__ */ new Set(["preprocess", "postprocess", "processAllTokens", "emStrongMask"])), m(H, "passThroughHooksRespectAsync", /* @__PURE__ */ new Set(["preprocess", "postprocess", "processAllTokens"])), H), Lt = class {
  constructor(...n) {
    m(this, "defaults", ce());
    ...
    return n.forEach((t) => {
      ...
      if (t.hooks) {
        ...
        for (let i in t.hooks) {
          ...
          U.passThroughHooks.has(i) ? r[o] = (a) => {
...
class Ft extends Qe {
  constructor(t, s) {
    super(t, s);
    N(this, E);
    N(this, K, new We(this));
    N(this, V, new Je(this));
    N(this, Y, new Ke(this));
  }
  async execute() {
    var r;
    const t = await this.getContext(He), s = this.args.unique ?? null;
    try {
      let i = null;
      if (s) {
        const { data: x } = await F(this, Y).requestItems([s]);
        x != null && x.length && (i = x[0].documentType.unique);
      }
      const l = (await F(this, K).requestAllowedChildrenOf(
        i,
        s
      )).data;
      ...
      for (const x of l.items) {
        const { data: S } = await F(this, V).requestItemsByDocumentType(x.unique);
      ...
      const { name: g, mediaUnique: w, pageTitle: T, pageTitleShort: I, pageDescription: L, itineraryContent: v } = d;
      if (!w || !g)
        return;
      console.log("Creating document with:", { name: g, pageTitle: T, pageTitleShort: I, pageDescription: L, itineraryContent: v == null ? void 0 : v.substring(0, 100) });
      const C = await (await this.getContext(Xe)).getLatestToken(), A = await fetch(
      ...
      console.log("Scaffold response:", D);
      const j = D.values ? [...D.values] : [], ne = (x, S) => {
        const _ = j.find((Ge) => Ge.alias === x);
        _ ? _.value = S : j.push({ alias: x, value: S });
      };
      ne("pageTitle", T), ne("pageTitleShort", I), ne("pageDescription", L), v && le(this, E, Ue).call(this, j, v);
      const me = {
        parent: s ? { id: s } : null,
        documentType: { id: k },
        template: D.template ? { id: D.template.id } : null,
        values: j,
        variants: [
          {
            name: g,
...
K = new WeakMap(), V = new WeakMap(), Y = new WeakMap(), E = new WeakSet(), /**
  ...
  try {
    ...
    for (const c of k) {
      ...
      if (console.log("Title value for block:", d == null ? void 0 : d.value), d && typeof d.value == "string" && d.value.toLowerCase().includes("suggested itinerary")) {
        ...
        if (g) {
          const w = le(this, E, Ze).call(this, s);
          g.value = {
            blocks: {
              contentData: [],
              settingsData: [],
...
export {
  Ft as CreateFromPdfEntityAction,
  Ft as default
};
//# sourceMappingURL=create-from-pdf-action-CviPVTiv.js.map
