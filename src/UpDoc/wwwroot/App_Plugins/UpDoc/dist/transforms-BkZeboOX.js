...
`) };
  ...
  fences(s) {
    ...
    if (e) {
      let r = e[0], n = Je(r, e[3] || "", this.rules);
...
`, s = s.substring(u.length + 1), a = !0), !a) {
          let b = this.rules.other.nextBulletRegex(f), U = this.rules.other.hrRegex(f), K = this.rules.other.fencesBeginRegex(f), J = this.rules.other.headingBeginRegex(f), fe = this.rules.other.htmlBeginRegex(f);
          for (; s; ) {
            let q = s.split(`
`, 1)[0], S;
            if (u = q, this.options.pedantic ? (u = u.replace(this.rules.other.listReplaceNesting, "  "), S = u) : S = u.replace(this.rules.other.tabCharGlobal, "    "), K.test(u) || J.test(u) || fe.test(u) || b.test(u) || U.test(u)) break;
            if (S.search(this.rules.other.nonSpaceChar) >= f || !u.trim()) c += `
` + S.slice(f);
            else {
              if (d || k.replace(this.rules.other.tabCharGlobal, "    ").search(this.rules.other.nonSpaceChar) >= 4 || K.test(k) || J.test(k) || U.test(k)) break;
              c += `
` + u;
            }
            !d && !u.trim() && (d = !0), h += q + `
...
function Ye(s) {
  ...
  try {
    ...
    return typeof e == "string" ? e : (console.warn("marked returned Promise, using fallback"), `<p>${s}</p>`);
  } catch (e) {
    return console.error("Markdown conversion failed:", e), `<p>${s.replace(/\n\n/g, "</p><p>").replace(/\n/g, "<br>")}</p>`;
  }
}
function et(s) {
