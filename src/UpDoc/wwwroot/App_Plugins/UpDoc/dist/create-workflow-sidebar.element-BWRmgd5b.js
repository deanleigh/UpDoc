import { U as X } from "./page-picker-modal.token-B0CgP9f1.js";
import { e as H } from "./workflow.service-D_fkSdCh.js";
import "./up-doc-pdf-thumbnail.element-N7K7a6d1.js";
import { css as P, property as Y, state as r, customElement as $, html as l, nothing as f } from "@umbraco-cms/backoffice/external/lit";
import { UmbLitElement as Z } from "@umbraco-cms/backoffice/lit-element";
import { UMB_MODAL_MANAGER_CONTEXT as w, UmbModalBaseElement as j } from "@umbraco-cms/backoffice/modal";
import { UMB_MEDIA_PICKER_MODAL as J } from "@umbraco-cms/backoffice/media";
import { UMB_AUTH_CONTEXT as C } from "@umbraco-cms/backoffice/auth";
import { UmbTextStyles as Q } from "@umbraco-cms/backoffice/style";
var V = Object.defineProperty, ee = Object.getOwnPropertyDescriptor, k = (e) => {
  throw TypeError(e);
}, _ = (e, t, a, o) => {
  for (var n = o > 1 ? void 0 : o ? ee(t, a) : t, d = e.length - 1, h; d >= 0; d--)
    (h = e[d]) && (n = (o ? h(t, a, n) : h(n)) || n);
  return o && n && V(t, a, n), n;
}, te = (e, t, a) => t.has(e) || k("Cannot " + a), ae = (e, t, a) => t.has(e) ? k("Cannot add the same private member more than once") : t instanceof WeakSet ? t.add(e) : t.set(e, a), b = (e, t, a) => (te(e, t, "access private method"), a), m, g, T, M;
let p = class extends Z {
  constructor() {
    super(...arguments), ae(this, m), this.label = "", this._mediaKey = null, this._mediaName = "", this._token = "";
  }
  async connectedCallback() {
    super.connectedCallback();
    const e = await this.getContext(C);
    this._token = await e.getLatestToken();
  }
  render() {
    return this._mediaKey ? l`
			<uui-card-media name=${this._mediaName || "PDF"}>
				<up-doc-pdf-thumbnail
					mediaKey=${this._mediaKey}
					page="1"
					width="300"
					token=${this._token}>
				</up-doc-pdf-thumbnail>
				<uui-action-bar slot="actions">
					<uui-button
						label="Remove"
						look="secondary"
						compact
						@click=${b(this, m, M)}>
						<uui-icon name="icon-trash"></uui-icon>
					</uui-button>
					<uui-button
						label="Change"
						look="secondary"
						compact
						@click=${b(this, m, g)}>
						<uui-icon name="icon-edit"></uui-icon>
					</uui-button>
				</uui-action-bar>
			</uui-card-media>
		` : l`
				<uui-button
					id="btn-add"
					look="placeholder"
					@click=${b(this, m, g)}
					label="Choose">
					<uui-icon name="icon-add"></uui-icon>
					Choose
				</uui-button>
			`;
  }
};
m = /* @__PURE__ */ new WeakSet();
g = async function() {
  const a = await (await this.getContext(w)).open(this, J, {
    data: {
      multiple: !1
    }
  }).onSubmit().catch(() => null);
  if (!a?.selection?.length) return;
  const o = a.selection[0];
  o && (this._mediaKey = o, this._mediaName = await b(this, m, T).call(this, o), this.dispatchEvent(
    new CustomEvent("change", {
      detail: { mediaKey: o },
      bubbles: !0,
      composed: !0
    })
  ));
};
T = async function(e) {
  try {
    const t = await fetch(
      `/umbraco/management/api/v1/media/item?id=${e}`,
      { headers: { Authorization: `Bearer ${this._token}` } }
    );
    return t.ok ? (await t.json())?.[0]?.name ?? "PDF" : "PDF";
  } catch {
    return "PDF";
  }
};
M = function() {
  this._mediaKey = null, this._mediaName = "", this.dispatchEvent(
    new CustomEvent("change", {
      detail: { mediaKey: null },
      bubbles: !0,
      composed: !0
    })
  );
};
p.styles = [
  P`
			:host {
				display: block;
			}

			#btn-add {
				text-align: center;
				width: 200px;
				height: 200px;
			}

			uui-icon {
				display: block;
				margin: 0 auto;
			}

			uui-card-media {
				width: 200px;
			}
		`
];
_([
  Y({ type: String })
], p.prototype, "label", 2);
_([
  r()
], p.prototype, "_mediaKey", 2);
_([
  r()
], p.prototype, "_mediaName", 2);
_([
  r()
], p.prototype, "_token", 2);
p = _([
  $("up-doc-pdf-picker")
], p);
var ie = Object.defineProperty, ne = Object.getOwnPropertyDescriptor, U = (e) => {
  throw TypeError(e);
}, c = (e, t, a, o) => {
  for (var n = o > 1 ? void 0 : o ? ne(t, a) : t, d = e.length - 1, h; d >= 0; d--)
    (h = e[d]) && (n = (o ? h(t, a, n) : h(n)) || n);
  return o && n && ie(t, a, n), n;
}, E = (e, t, a) => t.has(e) || U("Cannot " + a), N = (e, t, a) => (E(e, t, "read from private field"), a ? a.call(e) : t.get(e)), se = (e, t, a) => t.has(e) ? U("Cannot add the same private member more than once") : t instanceof WeakSet ? t.add(e) : t.set(e, a), s = (e, t, a) => (E(e, t, "access private method"), a), i, S, D, z, O, v, A, y, x, K, q, W, B, L, R, F, I, G;
const oe = [
  { value: "pdf", name: "PDF Document" },
  { value: "markdown", name: "Markdown" },
  { value: "web", name: "Web Page" },
  { value: "doc", name: "Word Document" }
];
let u = class extends j {
  constructor() {
    super(...arguments), se(this, i), this._activeTab = "source", this._name = "", this._sourceType = "", this._selectedMediaUnique = null, this._sourceUrl = "", this._extracting = !1, this._totalPages = 0, this._elementCount = 0, this._fileName = "", this._selectedPages = null, this._nameManuallyEdited = !1;
  }
  render() {
    return l`
			<umb-body-layout headline="Create Workflow">
				${s(this, i, W).call(this)}

				<div class="tab-content">
					${s(this, i, G).call(this)}
				</div>

				<uui-button
					slot="actions"
					id="close"
					label=${this.localize.term("general_close")}
					@click="${s(this, i, q)}"></uui-button>
				<uui-button
					slot="actions"
					id="save"
					look="primary"
					color="positive"
					label=${this.localize.term("general_create")}
					?disabled=${!N(this, i, x)}
					@click="${s(this, i, K)}"></uui-button>
			</umb-body-layout>
		`;
  }
};
i = /* @__PURE__ */ new WeakSet();
S = function(e) {
  return e.replace(/([a-z])([A-Z])/g, "$1-$2").replace(/[\s_]+/g, "-").toLowerCase();
};
D = function(e) {
  const a = e.target.value;
  a !== this._sourceType && (this._selectedMediaUnique = null, this._sourceUrl = ""), this._sourceType = a, !this._nameManuallyEdited && this.data?.blueprintName && this._sourceType && (this._name = `${s(this, i, S).call(this, this.data.blueprintName)}-${this._sourceType}`), this.requestUpdate();
};
z = async function(e) {
  const t = e.detail;
  if (this._selectedMediaUnique = t?.mediaKey ?? null, this._totalPages = 0, this._elementCount = 0, this._fileName = "", this._selectedPages = null, this._selectedMediaUnique) {
    this._extracting = !0;
    try {
      const o = await (await this.getContext(C)).getLatestToken(), n = await H(this._selectedMediaUnique, o);
      n && (this._totalPages = n.source.totalPages, this._elementCount = n.elements.length, this._fileName = n.source.fileName);
    } catch {
    } finally {
      this._extracting = !1;
    }
  }
};
O = async function() {
  if (!this._selectedMediaUnique || this._totalPages === 0) return;
  const a = await (await this.getContext(w)).open(this, X, {
    data: {
      mediaKey: this._selectedMediaUnique,
      totalPages: this._totalPages,
      selectedPages: this._selectedPages
    }
  }).onSubmit().catch(() => null);
  a !== null && (this._selectedPages = a.selectedPages);
};
v = async function(e) {
  const a = e.target.selection;
  this._selectedMediaUnique = a.length > 0 ? a[0] : null;
};
A = function(e) {
  this._name = e.target.value, this._nameManuallyEdited = !0;
};
y = function(e) {
  this._activeTab = e;
};
x = function() {
  return this._name.trim().length > 0 && this._sourceType.length > 0;
};
K = function() {
  N(this, i, x) && (this.value = {
    name: this._name.trim(),
    sourceType: this._sourceType,
    mediaUnique: this._selectedMediaUnique,
    sourceUrl: this._sourceUrl || null,
    documentTypeAlias: this.data.documentTypeAlias,
    blueprintId: this.data.blueprintUnique,
    blueprintName: this.data.blueprintName,
    selectedPages: this._selectedPages
  }, this._submitModal());
};
q = function() {
  this._rejectModal();
};
W = function() {
  return l`
			<uui-tab-group slot="navigation">
				<uui-tab
					label="Source"
					?active=${this._activeTab === "source"}
					orientation="horizontal"
					@click=${() => s(this, i, y).call(this, "source")}>
					<uui-icon slot="icon" name="icon-page-add"></uui-icon>
					Source
				</uui-tab>
				<uui-tab
					label="Destination"
					?active=${this._activeTab === "destination"}
					orientation="horizontal"
					@click=${() => s(this, i, y).call(this, "destination")}>
					<uui-icon slot="icon" name="icon-document"></uui-icon>
					Destination
				</uui-tab>
			</uui-tab-group>
		`;
};
B = function() {
  return this._fileName ? l`
			<div class="extraction-stats">
				<span class="extraction-filename">${this._fileName}</span>
				<div class="extraction-meta">
					<span><uui-icon name="icon-thumbnails-small"></uui-icon> ${this._totalPages} pages</span>
					<span><uui-icon name="icon-list"></uui-icon> ${this._elementCount} elements</span>
				</div>
			</div>
		` : f;
};
L = function() {
  switch (this._sourceType) {
    case "pdf":
      return l`
					<uui-box headline="Sample Document">
						<up-doc-pdf-picker @change=${s(this, i, z)}></up-doc-pdf-picker>
						${this._extracting ? l`<uui-loader-bar></uui-loader-bar>` : f}
						${s(this, i, B).call(this)}
					</uui-box>
				`;
    case "markdown":
      return l`
					<uui-box headline="Sample File">
						<umb-input-media max="1" @change=${s(this, i, v)}></umb-input-media>
					</uui-box>
				`;
    case "web":
      return l`
					<uui-box headline="Sample URL">
						<uui-input
							label="URL"
							placeholder="https://example.com/page"
							.value=${this._sourceUrl}
							@input=${(e) => this._sourceUrl = e.target.value}>
						</uui-input>
					</uui-box>
				`;
    case "doc":
      return l`
					<uui-box headline="Sample Document">
						<umb-input-media max="1" @change=${s(this, i, v)}></umb-input-media>
					</uui-box>
				`;
    default:
      return f;
  }
};
R = function() {
  if (this._totalPages === 0) return f;
  const e = this._selectedPages ? `${this._selectedPages.length} of ${this._totalPages} pages` : `All ${this._totalPages} pages`;
  return l`
			<uui-box headline="Pages">
				<span class="page-selection-label">${e}</span>
				<uui-button
					look="outline"
					label="Choose Pages"
					@click=${s(this, i, O)}
					class="full-width-button">
					<uui-icon name="icon-thumbnails-small"></uui-icon>
					Choose Pages
				</uui-button>
			</uui-box>
		`;
};
F = function() {
  return l`
			<uui-box headline="Workflow Name">
				<uui-input
					id="name"
					label="name"
					placeholder="e.g. group-tour-pdf"
					.value=${this._name}
					@input=${s(this, i, A)}>
				</uui-input>
			</uui-box>

			<uui-box headline="Format">
				<uui-select
					label="Select source format"
					.options=${[
    { name: "Choose a format...", value: "", selected: this._sourceType === "" },
    ...oe.map((e) => ({
      ...e,
      selected: this._sourceType === e.value
    }))
  ]}
					@change=${s(this, i, D)}>
				</uui-select>
			</uui-box>

			${s(this, i, L).call(this)}
			${s(this, i, R).call(this)}
		`;
};
I = function() {
  return l`
			<uui-box headline="Document Type">
				<div class="destination-value">
					<umb-icon name="icon-document-dashed-line"></umb-icon>
					<span>${this.data?.documentTypeName}</span>
				</div>
			</uui-box>

			<uui-box headline="Blueprint">
				<div class="destination-value">
					<umb-icon name="icon-blueprint"></umb-icon>
					<span>${this.data?.blueprintName}</span>
				</div>
			</uui-box>
		`;
};
G = function() {
  switch (this._activeTab) {
    case "source":
      return s(this, i, F).call(this);
    case "destination":
      return s(this, i, I).call(this);
  }
};
u.styles = [
  Q,
  P`
			.destination-value {
				display: flex;
				align-items: center;
				gap: var(--uui-size-space-3);
			}

			uui-input {
				width: 100%;
			}

			uui-select {
				width: 100%;
			}

			uui-box {
				margin-bottom: var(--uui-size-space-4);
			}

			.tab-content {
				display: flex;
				flex-direction: column;
			}

			.extraction-stats {
				margin-top: var(--uui-size-space-3);
				padding: var(--uui-size-space-3);
				border-radius: var(--uui-border-radius);
				background: var(--uui-color-surface-alt);
			}

			.extraction-filename {
				display: block;
				font-weight: 600;
				font-size: var(--uui-type-small-size);
				margin-bottom: var(--uui-size-space-2);
				word-break: break-all;
			}

			.extraction-meta {
				display: flex;
				gap: var(--uui-size-space-4);
				font-size: var(--uui-type-small-size);
				color: var(--uui-color-text-alt);
			}

			.extraction-meta span {
				display: flex;
				align-items: center;
				gap: var(--uui-size-space-1);
			}

			.page-selection-label {
				display: block;
				font-size: var(--uui-type-small-size);
				color: var(--uui-color-text-alt);
				margin-bottom: var(--uui-size-space-3);
			}

			.full-width-button {
				width: 100%;
			}

		`
];
c([
  r()
], u.prototype, "_activeTab", 2);
c([
  r()
], u.prototype, "_name", 2);
c([
  r()
], u.prototype, "_sourceType", 2);
c([
  r()
], u.prototype, "_selectedMediaUnique", 2);
c([
  r()
], u.prototype, "_sourceUrl", 2);
c([
  r()
], u.prototype, "_extracting", 2);
c([
  r()
], u.prototype, "_totalPages", 2);
c([
  r()
], u.prototype, "_elementCount", 2);
c([
  r()
], u.prototype, "_fileName", 2);
c([
  r()
], u.prototype, "_selectedPages", 2);
u = c([
  $("create-workflow-sidebar")
], u);
const be = u;
export {
  u as CreateWorkflowSidebarElement,
  be as default
};
//# sourceMappingURL=create-workflow-sidebar.element-BWRmgd5b.js.map
